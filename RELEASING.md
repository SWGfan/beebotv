...
Build and test first. Keep Android signing compatible with the existing website APK. Upload and verify the versioned Windows release before pointing the feed at it. Push the website, then verify its live metadata and Android APK hash. Set the verified release as latest for the homepage Windows download link. Preserve older releases for rollback.
